...
const ProfilePage = () => {
  ...
  const getUser = async () => {
    const response = await fetch(`https://socialmediaapp-iedb.onrender.com/users/${userId}`, {
      method: "GET",
    ...
    <Box>
      ...
      >
        <Box flexBasis={isNonMobileScreens ? "26%" : undefined}>
          <UserProfileCard userId={userId} picturePath={user.picturePath} />
          <Box m="2rem 0" />
        ...
        >
          ...
          <Box m="2rem 0" />
          <GetPostDetails userId={userId} isProfile />
        </Box>
      </Box>
    </Box>
  );
